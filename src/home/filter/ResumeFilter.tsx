...
export default function ResumeFilter() {
  ...
  const availableSkills: ChoiceSkill[] = useAppSelector(
    (state) => state.portfolio.resume?.skills?.values ?? [],
  )
  const availableBusinesses: ChoiceBusiness[] = useAppSelector(
    (state) => state.portfolio.resume?.business?.values ?? [],
  )

  function selectItem<T extends { name: string }>(item: T, items: T[], setItems: (items: T[]) => void) {
    ...
    <div className="flex flex-col items-center gap-8">
      <div className="flex flex-col items-center gap-4">
        <MultiSelect 
          ...
          selectItem={item => selectItem(item, currentSkills, setCurrentSkills)}
        ...
        <MultiSelect 
          ...
          selectItem={item => selectItem(item, currentBusiness, setCurrentBusiness)}
