...
interface SideProjectTimeframeDialogProps {
  dialogTitle: string
...
export default function SideProjectTimeframeDialog({
  isOpened,
  setOpened,
  initialTimeframe,
  dialogTitle,
  setTimeframe,
}: SideProjectTimeframeDialogProps) {
  ...
  useEffect(() => {
    ...
    setValidTimeframe(fromDateCorrect && toDateCorrect && datesCorrect)
      ...
      <div className="flex min-h-72 flex-col items-center gap-6 pt-6">
        <div>
          <label className="block text-sm font-medium text-gray-700">
            {t("resumeSideProject.timeframe.startDate")}
          </label>
          <CalendarInput date={fromDate} setDate={setFromDate} />
        </div>
        <div>
          <Checkbox
            label={t("resumeSideProject.timeframe.tillPresent")}
            checked={isToPresent}
        ...
        {!isToPresent && (
          <div>
            <label className="block text-sm font-medium text-gray-700">
              {t("resumeSideProject.timeframe.endDate")}
            </label>
